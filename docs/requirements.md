# 📋 TodayMemo 要件定義書 (Requirements)

## 1. アプリ概要
「今日やるべきこと」をシンプルに管理し、直感的に操作できるAndroid向けToDoアプリ。

## 2. 機能要件 (Functional Requirements)
- **タスク追加**: 入力欄に文字を入力し、追加ボタン（FAB）を押すと一覧に即座に追加される。
- **タスク表示**: 追加されたタスクをカード形式のリストで一覧表示する。
- **タスク検索**: 検索窓に入力したキーワードで、タスク一覧をリアルタイムに絞り込むことができる。
- **タスク並び替え**: 「作成順」「名前順」「完了状態順」の3つの条件でリストを並び替えることができる。
- **完了チェック**: チェックボックスをタップすることで、タスクの完了/未完了を切り替えられる。
- **タスク削除**: タスクを長押しすることで、確認ダイアログを表示した後に削除できる。
- **データの永続化**: アプリを終了しても、タスクの内容と完了状態が保存され、次回起動時に復元される。
- **リアクティブ更新**: データベースの変更が即座に画面へ反映される（LiveDataによる自動同期）。
- **常駐通知機能**: 未完了タスクを通知欄に常に表示。スワイプで消えず、タスクの状況に合わせて内容が自動更新される。
- **空状態の通知**: タスクが1件もない場合、ユーザーに「今日やることはありません！」というメッセージを表示する。

## 3. UI/UX要件 (UI/UX Requirements)
- **Material Design**: Googleのマテリアルデザインガイドラインに準拠したモダンな見た目。
- **CardView**: 各タスクを立体感のあるカード形式で表示し、視認性を高める。
- **Floating Action Button (FAB)**: 最も重要な操作である「追加」を、入力欄の横に配置（キーボードを考慮）。
- **リアルタイム・フィードバック**: 検索窓への入力や並び替えの選択と同時に、リストが即座に更新される。
- **視覚的フィードバック**:
    - 完了済みタスクには「取り消し線」を引き、文字色をグレーに変更する。
    - 削除前には必ず確認ダイアログを表示し、誤操作を防止する。
- **常駐型リマインダー**: 通知欄を見ればいつでも「今やるべきこと」が把握できる高い利便性。
- **没入型レイアウト**: システムのアクションバーを非表示にし、アプリ独自のタイトルエリアを最大活用。

## 4. 技術スタック (Technical Stack)
- **言語**: Kotlin
- **アーキテクチャ**: MVVM (Model-View-ViewModel) + Repository Pattern
- **データ監視**: LiveData (Reactive UI)
- **UI フレームワーク**: XML (View-based)
- **非同期処理**: Kotlin Coroutines
- **データ保存**: Room Database (SQLite)
- **通知機能**: NotificationManager / NotificationChannel / PendingIntent
- **ライブラリ**: 
    - `com.google.android.material:material` (UI部品)
    - `androidx.room:room-runtime` (データベース)
    - `androidx.lifecycle:lifecycle-livedata-ktx` (データ監視)

## 5. ターゲット・コンセプト
- **ターゲット**: Androidアプリ開発の基礎を学びたい初心者、およびシンプルなメモ帳を求めるユーザー。
- **コンセプト**: 「迷わない操作感」と「日常生活に溶け込む常駐型リマインダー」。
